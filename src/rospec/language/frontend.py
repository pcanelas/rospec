...
class TreeToROSpec(Interpreter):
    ...
    def configurable_type(self, args):
        ...
        if len(args) == 4:
            if not is_optional:
                raise ValueError(errors.DEFAULT_VALUE_NON_OPTIONAL)
            else:
                raise ValueError(errors.MISSING_DEFAULT_OPTIONAL.format(args=args))
    ...
    def service_action(self, args):
        ...
        if isinstance(expr, Identifier):
            ...
            raise ValueError(errors.UNEXPECTED_EXPR_TYPE.format(type=type(expr)))
